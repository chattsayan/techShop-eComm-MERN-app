...
dotenv.config();
const { PAYPAL_CLIENT_ID, PAYPAL_SECRET_KEY, PAYPAL_SANDBOX_URL } = process.env;
...
async function getPayPalAccessToken() {
  ...
  const url = `${PAYPAL_SANDBOX_URL}/v1/oauth2/token`;
...
export async function verifyPayPalPayment(paypalTransactionId) {
  ...
  const paypalResponse = await fetch(
    `${PAYPAL_SANDBOX_URL}/v2/checkout/orders/${paypalTransactionId}`,
    {
      headers: {
